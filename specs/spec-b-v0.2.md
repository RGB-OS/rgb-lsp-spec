...
With the §11.8 mirrors in place, the rationale sharpens to its final form: **mirrors give the vault settlement semantics; the v-asset gives it elasticity, continuity, and concurrency** — and nothing else. A client that forgoes those three runs the zero-credit pure-leaf profile of §11.9 with no v-asset at all; the overlay is a per-user dial, not a system-wide necessity.
...
**Interpretation.** A mirror is an HTLC adapted to unconfirmed funding — CSV in place of CLTV — so this extension is best understood as *Lightning run over the leaf channels, restricted to its sound subset*: relative timeouts only, and a star topology through the operator (leaf endpoints are fixed, so leaves never route third-party traffic). Under that reading the system has **one payment mechanism, not two**: overlay payments, internal transfers, and cross-asset swaps are all the same object — a multi-hop, possibly multi-asset HTLC payment in which hops over leaf channels use the CSV variant. For a pure member-to-member transfer the overlay leg is even optional: two mirrors plus the operator relaying the preimage suffice.

### 11.9 Pure-leaf mode (zero-credit client profile)

With mirrors available, the v-asset is no longer needed for *payments* — only for elasticity (absorbing receives beyond frozen leaf headroom as `pending`), continuity (carrying in-flight traffic across refreshes, where leaf channels are replaced and mirrors must resolve, [B-69]), and concurrency (overlay channels carry many simultaneous HTLCs; leaves cap mirrors at `N_mirror`). A client that wants none of those three can run **pure-leaf mode**, which is a policy restriction of this specification, not a variant of it:

- set `P_max = 0` ([B-29]) and accept a receive **only** as a §11.8 receive mirror that fits current leaf headroom — any other receive is failed back to the payer;
- send only via send mirrors ([B-70]);
- the overlay channel is OPTIONAL: it carries zero net balance if kept (every unit instantly mirror-settled) and MAY be omitted entirely — internal transfers use two mirrors with the operator relaying the preimage, and external Lightning payments use a mirror as the user-side leg of the operator's outward routing.

A pure-leaf client holds **zero credit at every instant of its existence**: every unit it has ever owned was settled, mirror-atomic property. The price is exactly the three jobs above: receives bounce at leaf capacity until the next rollover re-slices headroom, payments pause during the client's refresh, and concurrency is bounded by `N_mirror`. This profile is the maximally trust-minimized point of the design — operationally, covenant-less Ark with mirror payments — and pairs naturally with corridor shards (§9.6) whose frequent rollovers keep headroom fresh, and with the BTC profile (Appendix B), where it eliminates the v-asset entirely.

**[B-72]** A deployment offering pure-leaf mode MUST support mirror settlement without an overlay channel (internal preimage relay and mirror-leg external routing), and MUST propagate a capacity bounce as an ordinary payment failure to the payer (never as a recipient error); a pure-leaf Client MUST refuse any receive that is not mirror-settled within headroom. Deployments SHOULD disclose which of the three modes of §11.9's table they support; Clients MAY switch modes at a refresh boundary.

| Mode | Credit exposure | Capital | UX |
|---|---|---|---|
| Pure-leaf (`P_max = 0`, mirrors mandatory, overlay optional) | zero, ever | headroom sized to inter-rollover receive volume | bounces at capacity; pauses at refresh; `N_mirror` concurrency |
| Overlay + mirrors (RECOMMENDED default) | zero within headroom; capped `pending` beyond | ~1× + headroom | continuous |
| Overlay, base flow (§11.5) | seconds-wide window + capped `pending` | same | continuous |
...
- **Overlay choice (BTC-specific degree of freedom).** Unlike the USDT deployment (where a USD-denominated channel asset is required, §4.4 rationale), this profile MAY run the overlay either as synthetic `vBTC` (capital-free inbound capacity; receives are credit until settled — the standard rail/vault trade), as **plain BTC Lightning channels** (every received sat is immediately real, self-custodial channel balance; the LSP pays vanilla capital cost for inbound capacity), or — for pure-leaf clients (§11.9) — as **no overlay at all**, with all payments mirror-settled directly against leaves. With a plain-BTC overlay the tree still earns its place as the *stock* layer: channels are sized to payment throughput while balances park in the pooled tree, keeping real channel capacity small and recycled. A deployment MUST state which overlay modes it runs in its [B-58] capital model, since the pending semantics and the LSP's capital profile differ materially between them.
